...
class Main:
    ...
    def lowest_cost_walk_ui(self):
        x = int(input("Enter the start vertex: "))
        y = int(input("Enter the end vertex: "))
        if x not in self._graph.parse_vertices() or y not in self._graph.parse_vertices():
            print("The vertex input is incorrect!")
        elif x == y:
            print("The cost is 0 because the vertices coincide!")
        else:
            costs, walks = self._graph.floyd_warshall()
            if costs[x][y] == math.inf:
                print("There is no walk between the vertices entered!")
            else:
                print("The lowest cost is " + str(costs[x][y]) + "!")
                walk = [x]
                while x != y:
                    x = walks[x][y]
                    walk.append(x)
                print("The lowest cost walk is " + str(walk) + "!")
    ...
    def print_menu():
        ...
        print("19 - find lowest cost walk")
        print("----------------------------------")
    ...
    def start(self):
        commands = {
            ...
            "17": self.random_graph_ui, "18": self.use_copy_ui, "19": self.lowest_cost_walk_ui
                    }
        while True:
            self.print_menu()
