...
class Graph:
    ...
    def floyd_warshall(self):
        costs = [[math.inf] * self.no_vertices for i in range(self.no_vertices)]
        walks = [[-1] * self.no_vertices for i in range(self.no_vertices)]
        for i in range(self.no_vertices):
            for j in range(self.no_vertices):
                if i == j:
                    costs[i][j] = 0
                elif (i, j) in self._dict_info:
                    costs[i][j] = self.retrieve_info(i, j)
                    walks[i][j] = j
        for k in range(self.no_vertices):
            for i in range(self.no_vertices):
                for j in range(self.no_vertices):
